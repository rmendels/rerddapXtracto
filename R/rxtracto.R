...
if("latitude" %in% names(dimargs)){
  latSouth <- (dataCoordList$latitude[2] > dataCoordList$latitude[1] )
}
...
#map request limits to nearest ERDDAP coordinates
   newyIndex[1] <- which.min(abs(dataCoordList$latitude - ymin))
   newyIndex[2] <- which.min(abs(dataCoordList$latitude - ymax))
   newxIndex[1] <- which.min(abs(dataCoordList$longitude - xmin))
   newxIndex[2] <- which.min(abs(dataCoordList$longitude - xmax))
   if(tName %in% names(dataCoordList)){
   ...
   if (identical(newyIndex, oldyIndex) && identical(newxIndex, oldxIndex) && identical(newTimeIndex[1], oldTimeIndex[1])) {
     ...
     erddapY[1] <- dataCoordList$latitude[newyIndex[1]]
     erddapY[2] <- dataCoordList$latitude[newyIndex[2]]
     erddapX[1] <- dataCoordList$longitude[newxIndex[1]]
     erddapX[2] <- dataCoordList$longitude[newxIndex[2]]
     if(tName %in% names(dataCoordList)){

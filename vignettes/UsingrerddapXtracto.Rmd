...
The data extraction functions require obtaining information about the dataset using the function `rerddap::info()`, and possibly having to give the names of the coordinate variables, as these can not be assumed  (for example the zcoord could be in sigma coordinates).  More specifically:
...
The first step is to extract information about the dataset of interst from the <span style="color:red">ERDDAP</span> server being used. The needed informaiton include:
...
The track line with the locations colored according to the mean of the satellite `chlorophyll` around that point cna be plotted using `plotTrack()`. Positions where there was a tag location but no `chlorophyll` values are also shown. This example shows the use of the “plotColor” parameter to use the `cmocean` “algae” color palette.
...
The French agency IFREMER also has an <span style="color:red">ERDDAP</span> server, which has salinity data at 75 meters from the Global Ocean, Coriolis Observation Re-Analysis CORA4.1 model off the west coast of the United States.

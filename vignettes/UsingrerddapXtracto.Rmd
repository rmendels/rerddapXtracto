...
Besides the `rerddapXtracto` package, the examples below depend on the <span style="color:red">R</span> packages `akima`, `dplyr`, `ggplot2`,  `lubridate`, `mapdata`,  and `xts`.  These can be loaded beforehand (assuming they have been installed):
...
It should be emphasized that these other packages are used to manipulate and plot the data in <span style="color:red">R</span>, other packages could be used as well.  The use of `rerddapXtracto` does not depend on these other packages.

There are also several <span style="color:red">R</span> functions defined within the document that are used in other code examples.  These include `mapFrame()`, and `plotFrame()`.
...
We examine <span style="color:blue">chlorophyll</span> the "latest" data as of when the vignette was generated:
...
chlalogFrame <- mapFrame(VIIRS$longitude, VIIRS$latitude,
                       log(VIIRS$chla[, , 1]))
chlalogPlot <- plotFrame(chlalogFrame, xlim, ylim, "log(VIIRS chla)")
chlalogPlot
```
